...
SQL is a standard programming language which is used to create, add, retrieve, and manipulate the tables and data stored in relational databases.

2. Why do tables need a Primary Key?

Primary keys are a reliable way of identifying each database entry uniquely.

3. What is the name given to a table column that references the Primary Key on another table?

The column name is referred to as a foreign key which relates to a separate table on the database.

4. What do we need in order to have a _many to many_ relationship between two tables?

We need an intermediary table. It will hold the primary keys of multiple tables that have no direct connection to each other through foreign keys.
